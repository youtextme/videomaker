...
const scriptPath = argv[2] ?? join(root, "script.json");
...
const loaded = JSON.parse(readFileSync(scriptPath, "utf8"));
const scenes = loaded.map((s, i) => ({
  id: s.id ?? i + 1,
  variant: s.variant ?? "tower",
  accent: s.accent ?? "#D9A441",
  kicker: s.kicker ?? "VIDEOMAKER",
  title: s.title ?? "videomaker ready",
  sub: s.sub ?? "",
